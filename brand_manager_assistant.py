...
Create a complete deck structure with slide-by-slide content and recommendations."""
    ...
    def analyze_competitor(self, brand_name, category=None, focus_areas=None):
        """
...
3. Estimate media spend allocation across channels
...
- Breakdown by channel (TV, Digital, Social, OOH, Print, etc.)
- Share of voice vs category
- Spend trends (increasing/decreasing)
- ROI indicators
...
Base analysis on available market intelligence, public campaigns, and industry benchmarks."""

        category_text = f" in the {category} category" if category else ""
        focus_text = f"\n\nSpecific focus areas: {focus_areas}" if focus_areas else ""
        
        user_message = f"""Analyze the competitive intelligence for {brand_name}{category_text}.

Provide comprehensive analysis including:
1. Brand positioning
2. Recent campaigns (with specific examples)
3. Estimated media spend by channel
4. Strategic implications{focus_text}

Use your knowledge and reasoning to provide detailed, actionable insights."""
