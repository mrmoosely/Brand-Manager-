...
class BrandManagerAssistant:
    ...
    def _chat_with_tools(self, module_name, user_message, system_prompt, max_tokens=6000, use_web_search=False):
...
1. What specific competitor insights to highlight (use actual findings)
2. Which campaigns to showcase (use actual campaign names and dates)
3. What data visualizations to create (use actual numbers)
4. What strategic implications to emphasize (use actual recommendations)
5. Specific visual suggestions based on the content""",
            
            "creative_review": """Based on the creative review, create a detailed Gamma.ai prompt for a presentation deck. Include:

1. What specific creative elements to showcase
2. Which evaluation scores to highlight (use actual scores)
3. What improvement recommendations to feature (use actual suggestions)
4. Which persona insights to emphasize (use actual persona feedback)
5. Specific before/after suggestions based on the review"""
        }
        
        template = prompt_templates.get(deck_type, prompt_templates["competitive_intelligence"])
        
        user_message = f"""Here is the analysis content:

{analysis_content}

{template}

Create a comprehensive, SPECIFIC Gamma.ai prompt that someone could paste directly into Gamma.ai 
to create a presentation about this exact analysis. Use actual data, names, and findings from above."""
...
- When using web search, look for "2025", "recent", "latest", "new campaign"
- Always cite specific sources with dates and URLs when referencing campaigns or news
...
- Include source URLs for all campaign information
- Format citations as: [Source: Publication Name, Date, URL]
- Cite press releases, news articles, and official announcements
- If using web search results, include the specific article/source for each claim

Your role is to:
1. Search the web for the MOST RECENT marketing campaigns and news
2. Analyze competitor's current brand positioning and messaging
3. Identify their MOST RECENT marketing campaigns and creative strategies
4. Assess media spend and channel mix for CURRENT activities
5. Provide strategic implications and competitive threats
6. Recommend counter-strategies based on RECENT competitor moves
...
- Include specific dates when discussing campaigns (e.g., "Q4 2025 campaign launched Nov 1")
- Cite every major claim with source, date, and URL
