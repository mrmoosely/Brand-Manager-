...
def show_subscription_page():
    """Professional subscription management page with beautiful pricing cards"""
    ...
    st.markdown(f"""
        <div style='background: #F9FAFB; 
                    ...
                    margin-bottom: 3rem;
                    text-align: center;'>
            <div style='color: #6B7280; font-size: 0.875rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 0.5rem;'>
                Current Plan
            </div>
            <div style='color: #1F2937; font-size: 1.5rem; font-weight: 700;'>
                {tier_info["name"]} - {tier_info["price"]}
            </div>
            <div style='color: #2E5CFF; font-size: 1.125rem; margin-top: 0.5rem; font-weight: 600;'>
                {remaining} of {tier_info["analyses_per_month"]} analyses remaining this month
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    for idx, (tier_key, tier_data) in enumerate(SUBSCRIPTION_TIERS.items()):
        with cols[idx]:
            ...
            border_color = "#2E5CFF" if is_pro else "#E5E7EB"
            ...
            st.markdown(f"""
                <div style='background: white; 
                            ...
                            height: 100%; 
                            position: relative;
                            box-shadow: {"0 10px 25px -5px rgba(46, 92, 255, 0.2)" if is_pro else "0 1px 3px 0 rgba(0, 0, 0, 0.1)"};'>
                    
                    {'<div style="position: absolute; top: -14px; left: 50%; transform: translateX(-50%); background: linear-gradient(135deg, #F59E0B 0%, #D97706 100%); color: white; padding: 0.375rem 1rem; border-radius: 1rem; font-size: 0.75rem; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase;">MOST POPULAR</div>' if is_pro else ''}
                    
                    <div style='text-align: center; margin-bottom: 1.5rem;'>
                        <h3 style='color: #1F2937; margin-bottom: 1rem; font-size: 1.5rem; font-weight: 700;'>
                            {tier_data["name"]}
                        </h3>
                        <div style='margin-bottom: 0.5rem;'>
                            <span style='font-size: 3rem; 
                                       font-weight: 700; 
                                       color: #2E5CFF;'>
                                {tier_data["price"].split('/')[0]}
                            </span>
                            <span style='font-size: 1.125rem; color: #6B7280; font-weight: 500;'>/mo</span>
                        </div>
                    </div>
                    ...
                    <ul style='list-style: none; 
                        ...
                        {''.join([f'<li style="padding: 0.625rem 0; padding-left: 1.75rem; position: relative; color: #374151; font-size: 0.95rem;"><span style="position: absolute; left: 0; color: #10B981; font-weight: bold; font-size: 1.125rem;">✓</span> {feature}</li>' for feature in tier_data["features"]])}
                    </ul>
                </div>
            """, unsafe_allow_html=True)
            ...
            if is_current:
                ...
                if st.button(
                    f"Upgrade to {tier_data['name']}" if current_tier != "team" else f"Switch to {tier_data['name']}",
                    key=f"upgrade_{tier_key}",
                    use_container_width=True,
                    type="primary" if is_pro else "secondary"
                ):
                    st.info("🔗 **Payment Link**")
                    st.markdown(f"[Complete Secure Payment on Stripe →]({tier_data['stripe_link']})")
                    st.markdown("""
                        <div style='background: #EFF6FF; 
                            ...
                            <div style='color: #1E3A8A; font-size: 0.875rem;'>
                                Email your receipt to <strong>upgrade@brandmanagerai.com</strong><br/>
                                We'll upgrade your account within 24 hours.
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
    ...
    st.markdown("""
        <div style='text-align: center; margin-bottom: 2rem;'>
            <h2 style='color: #1F2937; margin-bottom: 0.5rem;'>Why Brand Manager AI?</h2>
    ...
    with col3:
        st.markdown("""
            <div style='text-align: center; padding: 1.5rem;'>
                ...
                <p style='color: #6B7280; font-size: 0.95rem;'>
                    $120K+ annual value for just $588/year
                </p>
            </div>
        """, unsafe_allow_html=True)
